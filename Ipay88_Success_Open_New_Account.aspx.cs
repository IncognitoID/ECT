...
public partial class Ipay88_Success_Open_New_Account : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            if (StatusFinal == "RECEIVEOK")
            {
                ...
                using (SqlCommand cmd = new SqlCommand("Load_Order_Details_Temporarily", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
    ...
    {
        using (SqlCommand cmd = new SqlCommand("Insert_New_Member_Successful_Register_Open_New_Account", con))
        {
            con.Open();
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Connection = con;
    ...
    protected void Calculate_Direct_Profit(string orderno)
    {
        ...
        if (user_details.Rows.Count > 0)
        {
            foreach (DataRow dr in user_details.Rows)
            {
                if (dr["First_Referal_ID"].ToString() != "-")
                {
                    ...
                    normal_item_total_price = (Convert.ToDecimal(dr["Normal_Price"].ToString()) * Convert.ToDecimal(dr["Qty"].ToString())) * (Convert.ToDecimal(dr["Product_Direct_Profit_Level_1"].ToString()) / Convert.ToDecimal(100));
                    promotion_item_total_price = (Convert.ToDecimal(dr["DC_Price"].ToString()) * Convert.ToDecimal(dr["DC_Qty"].ToString())) * (Convert.ToDecimal(dr["Product_Promotion_Direct_Profit_Level_1"].ToString()) / Convert.ToDecimal(100)); ;
                    ...
                    using (SqlCommand cmd = new SqlCommand("Insert_Direct_Profit", con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@OrderNo", orderno);
                        cmd.Parameters.AddWithValue("@Transaction_Amount", dr["Total_Price"].ToString());
                        cmd.Parameters.AddWithValue("@Transaction_Rate", dr["Product_Direct_Profit_Level_1"].ToString());
                    ...
                    if (member_upline.Rows.Count > 0)
                    {
                        ...
                        {
                            normal_item_total_price = (Convert.ToDecimal(dr["Normal_Price"].ToString()) * Convert.ToDecimal(dr["Qty"].ToString())) * (Convert.ToDecimal(dr["Product_Direct_Profit_Level_2"].ToString()) / Convert.ToDecimal(100));
                            promotion_item_total_price = (Convert.ToDecimal(dr["DC_Price"].ToString()) * Convert.ToDecimal(dr["DC_Qty"].ToString())) * (Convert.ToDecimal(dr["Product_Promotion_Direct_Profit_Level_2"].ToString()) / Convert.ToDecimal(100)); ;
                            ...
                            using (SqlCommand cmd = new SqlCommand("Insert_Direct_Profit", con))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@OrderNo", orderno);
                                cmd.Parameters.AddWithValue("@Transaction_Amount", dr["Total_Price"].ToString());
                                cmd.Parameters.AddWithValue("@Transaction_Rate", dr["Product_Direct_Profit_Level_2"].ToString());
        ...
        using (SqlCommand cmd = new SqlCommand("Insert_Transaction_BV", con))
        {
            ...
            if (profit_center == "3")
            {
                cmd.Parameters.AddWithValue("@Split_Bill", "Y");
            }
            else
            {
