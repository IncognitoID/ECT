...
public partial class MyProfile : System.Web.UI.Page
{
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (cookieExists == true)
                {
                    if (Request.Cookies["userid"].Value != null && Request.Cookies["userid"].Value != "")
                    {
                        ...
                        Load_Member_Month_Bv_Record(Request.Cookies["userid"].Value);
    ...
    protected void Load_Member_Month_Bv_Record(string memberid)
    {
        using (SqlConnection con = new SqlConnection(ConnectionString))
        {
            using (SqlCommand cmd = new SqlCommand("Load_Member_Month_Bv", con))
            {
                ...
                if (idr.HasRows == true)
                {
                    ...
                    grd_bv_month_record.DataSource = v;
                    grd_bv_month_record.DataBind();
                    grd_bv_month_record.HeaderRow.TableSection = TableRowSection.TableHeader;
                }
                else
                {
                    DataTable v = new DataTable();
                    ...
                    grd_bv_month_record.DataSource = v;
                    grd_bv_month_record.DataBind();
                    int columnCount = grd_bv_month_record.Rows[0].Cells.Count;

                    grd_bv_month_record.Rows[0].Cells.Clear();
                    grd_bv_month_record.Rows[0].Cells.Add(new TableCell());
                    grd_bv_month_record.Rows[0].Cells[0].ColumnSpan = columnCount;
                    grd_bv_month_record.Rows[0].Cells[0].Text = "No Record Found.";
                    grd_bv_month_record.HeaderRow.TableSection = TableRowSection.TableHeader;
    ...
    protected void btn_previousbv_Click(object sender, EventArgs e)
    {
        ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "Show_BvMonth();", true);
    }
}