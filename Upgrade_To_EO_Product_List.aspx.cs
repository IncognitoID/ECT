...
public partial class Upgrade_To_EO_Product_List : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            if (cookieExists == true)
            {
                ...
                {
                    Load_Member_BV(Request.Cookies["userid"].Value);
                    Load_All_Package();
                    Load_All_Product();
                    Load_Cart_Note();
                }
        ...
        using (SqlCommand cmd = new SqlCommand("Load_Member_Details", con))
        {
            ...
            if (idr.HasRows == true)
            {
                ...
                if (Request.QueryString["Package"] == "EO1")
                {
                    ...
                    string lblPackageText = "Package 2000BV";
                    string compulsoryText = "Package 2000BV (Compulsory)";
                    string singleText = "A La Carte";

                    // Update values for Chinese language
                    if (language == "Chinese")
                    {
                        lblPackageText = "配套 2000BV";
                        compulsoryText = "配套 2000BV (必须)";
                        singleText = "单品";
                    }
                ...
                else if (Request.QueryString["Package"] == "EO5")
                {
                    if (v.Rows[0]["Member_Category"].ToString() == "BO")
                    {
                        if (v.Rows[0]["Profit_Center"].ToString() == "1")
                        {
                            ...
                            // Default values for package
                            string lblPackageText = "Package 4000BV";
                            string compulsoryText = "Package 4000BV (Compulsory)";
                            string singleText = "A La Carte";

                            // Update values for Chinese language
                            if (language == "Chinese")
                            {
                                lblPackageText = "配套 4000BV";
                                compulsoryText = "配套 4000BV (必须)";
                                singleText = "单品";
                            }
                ...
                else if (Request.QueryString["Package"] == "EO9")
                {
                    if (v.Rows[0]["Member_Category"].ToString() == "BO")
                    {
                        if (v.Rows[0]["Profit_Center"].ToString() == "1")
                        {
                            ...
                            // Default values for package
                            string lblPackageText = "Package 8000BV";
                            string compulsoryText = "Package 8000BV (Compulsory)";
                            string singleText = "A La Carte";

                            // Update values for Chinese language
                            if (language == "Chinese")
                            {
                                lblPackageText = "配套 8000BV";
                                compulsoryText = "配套 8000BV (必须)";
                                singleText = "单品";
                            }
                        ...
                        else if (v.Rows[0]["Profit_Center"].ToString() == "3")
                        {
                            ...
                            // Default values for package
                            string lblPackageText = "Package 6000BV";
                            string compulsoryText = "Package 6000BV (Compulsory)";
                            string singleText = "A La Carte";

                            // Update values for Chinese language
                            if (language == "Chinese")
                            {
                                lblPackageText = "配套 6000BV";
                                compulsoryText = "配套 6000BV (必须)";
                                singleText = "单品";
                            }
    ...
    {
        SqlCommand cmdproduct = new SqlCommand("Load_Front_End_Registration_All_Product_v2", con);
    ...
    protected void Load_Product_Variation(string product_code, string quantity)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("Load_Front_End_Activation_Product_Details", con))
        {
            con.Open();
            cmd.CommandType = CommandType.StoredProcedure;
    ...
    protected void btn_continue_Click(object sender, EventArgs e)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("Load_Registration_Member_Checkout_Cart", con))
        {
            ...
            {
                ShowMessage_warning("Please add item in your cart, before checkout", MessageType.Warning);
                return;
            }
        ...
        if (v.Rows.Count > 0)
        {
            using (SqlCommand cmd = new SqlCommand("Check_Upgrade_Level_Member_Cart", con))
            {
                ...
                {
                    Response.Redirect("Upgrade_To_EO_Checkout.aspx?id=" + Request.Cookies["userid"].Value + "&Agent_Level=EO&Package=" + Request.QueryString["Package"].ToString());
                }
