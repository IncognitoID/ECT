...
public partial class Wallet : System.Web.UI.Page
{
    ...
    protected void Load_Member_Wallet_Details(string memberid)
    {
        using (SqlConnection con = new SqlConnection(ConnectionString))
        {
            ...
            //product wallet details

            using (SqlCommand cmd = new SqlCommand("Load_Product_Wallet_Details", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Memberid", memberid);
                con.Open();
                ...
                if (dt.Rows.Count > 0)
                {
                    lbl_product_wallet_amount.Text = "RM " + dt.Rows[0]["Product_Wallet_Balance"].ToString();
                }
    ...
    protected void Load_Member_Bonus_Details(string memberid)
    {
        using (SqlConnection con = new SqlConnection(ConnectionString))
        {
            using (SqlCommand cmd = new SqlCommand("Load_Member_Bonus_Details", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Memberid", memberid);
                cmd.Parameters.AddWithValue("@Start_Date", DateTime.UtcNow.AddHours(8).AddDays(-3).ToString("yyyy-MM-dd"));
    ...
    protected void btn_view_productwallet_transaction_Click(object sender, EventArgs e)
    {
        Response.Redirect("Transaction_History_PW.aspx");
    }
}