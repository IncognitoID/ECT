...
public partial class Redemption_Listing : System.Web.UI.Page
{
    ...
    protected void LoadCartNo()
    {
        ...
        if (cookieExists == true)
        {
            if (Request.Cookies["userid"].Value != null && Request.Cookies["userid"].Value != "")
            {
                ...
                using (SqlCommand cmd = new SqlCommand("Get_User_Redemption_CartItem", con))
                {
    ...
    protected void Load_All_Redemption()
    {
        SqlCommand cmdproduct = new SqlCommand("Load_Front_End_All_Redemption_Product", con);
    ...
    protected void Load_Product_Variation(string product_code, string quantity)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("Load_Front_End_Redemption_Item_Details", con))
        {
            con.Open();
            cmd.CommandType = CommandType.StoredProcedure;
    ...
    protected void AddItemtoCart(string itemcode, string variation)
    {
        using (SqlCommand cmdcheckvariation = new SqlCommand("Check_Redemption_Cart", con))
        {
            ...
            if (v2.Rows.Count > 0)
            {
                using (SqlCommand cmdinsertcart = new SqlCommand("Update_Redemption_Cart", con))
                {
            ...
            {
                using (SqlCommand cmdinsertcart = new SqlCommand("Insert_Redemption_Cart", con))
                {
                    cmdinsertcart.CommandType = CommandType.StoredProcedure;
                    cmdinsertcart.Parameters.AddWithValue("@MemberID", Request.Cookies["userid"].Value);
                    cmdinsertcart.Parameters.AddWithValue("@ItemCode", itemcode);
    ...
    {
        Response.Redirect("Redemption_Checkout.aspx");
    }
}