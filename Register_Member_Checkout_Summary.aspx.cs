...
public partial class Register_Member_Checkout_Summary : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            bool cookieExists = HttpContext.Current.Request.Cookies["Registration_Payment_order_no"] != null;

            if (cookieExists == true)
            {
                if (Request.Cookies["Registration_Payment_order_no"].Value != null && Request.Cookies["Registration_Payment_order_no"].Value != "")
                {
                    Load_Ecentra_Bank_Details();
                    Load_Order_Details(Request.Cookies["Registration_Payment_order_no"].Value);
    ...
    {
        using (SqlCommand cmd = new SqlCommand("Load_Registration_Order_History_Details", con))
        {
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            if (idr.HasRows)
            {
                ...
                itemSubtotal.InnerText = "RM " + Convert.ToDecimal(g.Rows[0]["Item_Total"].ToString()).ToString("N2");
                ...
                totalPayment.InnerText = "RM " + Convert.ToDecimal(g.Rows[0]["Total_Amount"].ToString()).ToString("N2");
            }
            con.Close();
        }
    }
        ...
        else
        {
            ...
            if (cookieExists == true)
            {
                if (Request.Cookies["Registration_Payment_order_no"].Value != null && Request.Cookies["Registration_Payment_order_no"].Value != "")
                {
                    ...
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@OrderNo", Request.Cookies["Registration_Payment_order_no"].Value);
                        con.Open();
                        SqlDataReader idr = cmd.ExecuteReader();
                    ...
                    if (ddlPaymentMethod.SelectedValue == "online")
                    {
                        ...
                        payment_refno = Request.Cookies["Registration_Payment_order_no"].Value;
                        payment_amount = order_total_amount.ToString();
                        payment_remark = "";

                        if (order_total_amount > 0)
                        {
                            Response.Cookies["Registration_Payment_order_no"].Expires = DateTime.Now.AddMonths(-1);
                            Response.Cookies["Registration_Payment_order_no"].Value = string.Empty;
                            Response.Cookies.Add(new HttpCookie("Registration_Payment_order_no", string.Empty));
                    ...
                    else if (ddlPaymentMethod.SelectedValue == "ewallet")
                    {
                        ...
                        if (Payment_Method_Success)
                        {
                            ...
                            string order_no = "";
                            order_no = Request.Cookies["Registration_Payment_order_no"].Value;
                            ...
                            using (SqlCommand cmd = new SqlCommand("Update_Registration_Wallet_Payment_Order_Details", con))
                            {
                                cmd.CommandType = CommandType.StoredProcedure;
                                ...
                                cmd.Parameters.AddWithValue("@Deduct_Member_ID", Request.Cookies["userid"].Value);
                                con.Open();
                                cmd.ExecuteNonQuery();
                                con.Close();
                            }
                            ...
                            {
                                Response.Cookies["Registration_Success_order_no"].Value = order_no;
                                Response.Cookies["Registration_Payment_order_no"].Expires = DateTime.Now.AddMonths(-1);
                                Response.Cookies["Registration_Payment_order_no"].Value = string.Empty;
                                Response.Cookies.Add(new HttpCookie("Registration_Payment_order_no", string.Empty));
                                Response.Redirect("Registration_Payment_Success.aspx");
                            }
                        }
                            ...
                            foreach (DataRow row in user_details.Rows)
                            {
                                ...
                                if (ddlPaymentMethod.SelectedValue == "manual")
                                {
                                    foreach (HttpPostedFile uploadedFile in FileUpload1.PostedFiles)
                                    {
                                        ...
                                        SaveImageLinkToDatabase(row["Order_No"].ToString(), imagelink);
                                    }
                                }

                                #endregion

                                using (SqlCommand cmd = new SqlCommand("Update_Registration_Payment_Order_Details", con))
                                {
                                    cmd.CommandType = CommandType.StoredProcedure;
                                    cmd.Parameters.AddWithValue("@Order_No", row["Order_No"].ToString());
                                    ...
                                    cmd.Parameters.AddWithValue("@Payment_Amount", row["Payment_Amt"].ToString());
                                    cmd.Parameters.AddWithValue("@Deduct_Member_ID", Request.Cookies["userid"].Value);
                                    con.Open();
                                    cmd.ExecuteNonQuery();
                                    con.Close();
    ...
    {
        using (SqlCommand cmd = new SqlCommand("Insert_New_Member_Successful_Register_Manual_Payment", con))
        {
            con.Open();
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Connection = con;
