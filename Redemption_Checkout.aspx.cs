...
public partial class Redemption_Checkout : System.Web.UI.Page
{
    ...
    protected void Calculate_Shipping_Fees(string memberid)
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("Load_Member_Redemption_Cart", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Memberid", memberid);
                con.Open();
            ...
            if (got_item)
            {
                foreach (DataRow dr in cart.Rows)
                {
                    ...
                    {
                        total_weight = total_weight + (Convert.ToDecimal(dr["Product_Weight"].ToString()) * (Convert.ToDecimal(dr["RP_Quantity"].ToString())));
                    }
                    else
                    {
                        total_weight = total_weight + (Convert.ToDecimal(dr["Variation_Weight"].ToString()) * (Convert.ToDecimal(dr["RP_Quantity"].ToString())));
                    }
                }
    ...
    protected void Load_Shipping_Discount(string memberid)
    {
        ...
        lbl_total_amout.Text = "RM " + shipping_fees.ToString("###,###,##0.00");
    ...
    protected void rpt_cart_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            {
                lbl_total_weight.Text = (Convert.ToDecimal(drv.Row["RP_Quantity"].ToString()) * Convert.ToDecimal(drv.Row["Product_Weight"].ToString())).ToString("###,###,##0.00");
            ...
            {
                lbl_total_weight.Text = (Convert.ToDecimal(drv.Row["RP_Quantity"].ToString()) * Convert.ToDecimal(drv.Row["Variation_Weight"].ToString())).ToString("###,###,##0.00");
            ...
            lbl_total_qty.Text = Convert.ToInt32(drv.Row["RP_Quantity"].ToString()).ToString();
    ...
    protected void rpt_cart_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            ...
            Label lbl_qty = e.Item.FindControl("lbl_rp_qty") as Label;
        ...
        {
            using (SqlCommand cmd = new SqlCommand("Delete_Redemption_Cart_Item", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
    ...
    protected void Load_Product_Variation(string Cart_ID, string quantity)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("Load_Front_End_Checkout_Redemption_Details", con))
        {
            con.Open();
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (string.IsNullOrEmpty(dt.Rows[0]["Cart_Variation_Value"].ToString()))
                {
                    ...
                    int total_qty = 0;
                    total_qty = Convert.ToInt32(dt.Rows[0]["RP_Quantity"].ToString());
    ...
    {
        using (SqlCommand cmd = new SqlCommand("Load_Front_End_Redemption_Variation_Value", con))
        {
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@Product_Code", product_code);
    ...
    public static string ProcessArray(string[] myArray, string id)
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("Load_Redemption_Variation_Details", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
    ...
    {
        using (SqlCommand cmdcheckvariation = new SqlCommand("Check_Redemption_Cart", con))
        {
            con.Open();
            ...
            if (v2.Rows.Count > 0)
            {
                using (SqlCommand cmdinsertcart = new SqlCommand("Update_Redemption_Cart_Checkout", con))
                {
            ...
            {
                using (SqlCommand cmdinsertcart = new SqlCommand("Insert_Redemption_Cart_Checkout", con))
                {
                    cmdinsertcart.CommandType = CommandType.StoredProcedure;
                    cmdinsertcart.Parameters.AddWithValue("@MemberID", Request.Cookies["userid"].Value);
                    cmdinsertcart.Parameters.AddWithValue("@ItemCode", itemcode);
    ...
    {
        DataTable v = new DataTable();
        using (SqlCommand cmd = new SqlCommand("Load_Member_Redemption_Checkout_Cart", con))
        {
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        foreach (DataRow dr in v.Rows)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("Check_Redemption_Item_Qty", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
    ...
    protected void Insert_Order(string member_id)
    {
        ...
        while (orderid == "")
        {
            ...
            orderid = "MYRP" + uniqueId;

            using (SqlCommand cmd = new SqlCommand("Check_Redemption_Order_No", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        foreach (DataRow row in v.Rows)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("Insert_Redemption_Order_Item", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                con.Open();
                ...
                {
                    total_weight = (Convert.ToDecimal(row["RP_Quantity"].ToString())) * Convert.ToDecimal(row["Product_Weight"].ToString());
                ...
                {
                    total_weight = (Convert.ToDecimal(row["RP_Quantity"].ToString())) * Convert.ToDecimal(row["Variation_Weight"].ToString());
            ...
            total_qty = total_qty + Convert.ToInt32(row["RP_Quantity"].ToString());
        ...
        using (SqlCommand cmd2 = new SqlCommand("Insert_Redemption_Order", con))
        {
            con.Open();
            ...
            cmd2.Parameters.AddWithValue("@ItemTotal", itemtotal);
        ...
        cmd_cart.CommandText = "DELETE FROM MF_Member_Redemption_Cart WHERE IDs IN (SELECT CartID FROM Redemption_Order_Item WHERE Order_No = '" + orderid + "')";
        ...
        using (SqlCommand cmd = new SqlCommand("Redemption_Checkout_Load_Member_Details", con))
        {
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@Member_ID", member_id);
            con.Open();
        ...
        if (total_price > 0)
        {
            Response.Redirect("Ipay88_Request_Redemption_Order.aspx?RefNo=" + payment_refno + "&Amount=" + payment_amount + "&Username=" + payment_username + "&UserEmail=" + payment_useremail + "&UserContact=" + payment_userphone + "&Remark=" + payment_remark);
        }
        else if (total_price == 0 && final_total_rp_used > 0)
        {
            Response.Cookies["redemption_order_no"].Value = payment_refno;
            Response.Redirect("Redemption_Payment_Success.aspx");
        }
    ...
    {
        Response.Redirect("Redemption_Listing.aspx");
    }
}