...
public partial class Mobile_Stockist_Checkout : System.Web.UI.Page
{
    ...
    protected void Calculate_Shipping_Fees(string memberid)
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("Load_Mobile_Stockist_Cart", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Memberid", memberid);
                con.Open();
    ...
    protected void rpt_cart_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("Delete_Mobile_Stockist_Cart_Item", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
    ...
    protected void Load_Product_Variation(string Cart_ID, string quantity)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("Load_Front_End_Mobile_Stockist_Checkout_Product_Details", con))
        {
            con.Open();
            cmd.CommandType = CommandType.StoredProcedure;
    ...
    {
        using (SqlCommand cmdcheckvariation = new SqlCommand("Check_Mobile_Stockist_Cart", con))
        {
            con.Open();
            ...
            if (v2.Rows.Count > 0)
            {
                using (SqlCommand cmdinsertcart = new SqlCommand("Update_Mobile_Stockist_Cart_Checkout", con))
                {
            ...
            {
                using (SqlCommand cmdinsertcart = new SqlCommand("Insert_Mobile_Stockist_Cart_Checkout", con))
                {
                    cmdinsertcart.CommandType = CommandType.StoredProcedure;
                    cmdinsertcart.Parameters.AddWithValue("@MemberID", Request.Cookies["userid"].Value);
                    cmdinsertcart.Parameters.AddWithValue("@ItemCode", itemcode);
    ...
    protected void btn_place_order_Click(object sender, EventArgs e)
    {
        ...
        if (cookieExists == true)
        {
            if (Request.Cookies["userid"].Value != null && Request.Cookies["userid"].Value != "")
            {
                ...
                if (GotItem == true && ItemNotAvailable == false && Country_Available == true)
                {
    ...
    {
        Response.Redirect("Mobile_Stockist_Product_Listing.aspx");
    }
}