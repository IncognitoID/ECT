...
public partial class Activate_Account_Product_List : System.Web.UI.Page
{
                ...
                if (v.Rows[0]["Profit_Center"].ToString() == "1")
                {
                    ...
                    if (cookieExists == true)
                    {
                        if (Request.Cookies["language"].Value != null && Request.Cookies["language"].Value != "")
                        {
                            ...
                            {
                                lbl_pacakge.InnerText = "配套 1000BV";
                                lbl_package_2.InnerText = "配套 1000BV";
                                ListItem newItem = new ListItem("配套 1000BV (必须)", "Package");
                            ...
                            {
                                lbl_pacakge.InnerText = "Package 1000BV";
                                lbl_package_2.InnerText = "Package 1000BV";
                                ListItem newItem = new ListItem("Package 1000BV (Compulsory)", "Package");
                                newItem.Selected = true;
                                rbtn_package.Items.Add(newItem);
                ...
                else
                {
                    ...
                    if (cookieExists == true)
                    {
                        if (Request.Cookies["language"].Value != null && Request.Cookies["language"].Value != "")
                        {
                            if (Request.Cookies["language"].Value == "Chinese")
                            {
                                lbl_pacakge.InnerText = "配套 3000BV";
                                lbl_package_2.InnerText = "配套 3000BV";
                                ListItem newItem = new ListItem("配套 3000BV (必须)", "Package");
                                newItem.Selected = true;
                                rbtn_package.Items.Add(newItem);
                                ListItem newItem2 = new ListItem("单品", "Single");
                                rbtn_package.Items.Add(newItem2);
                            ...
                            {
                                lbl_pacakge.InnerText = "Package 3000BV";
                                lbl_package_2.InnerText = "Package 3000BV";
                                ListItem newItem = new ListItem("Package 3000BV (Compulsory)", "Package");
                                newItem.Selected = true;
                                rbtn_package.Items.Add(newItem);
                                ListItem newItem2 = new ListItem("A La Carte", "Single");
                                rbtn_package.Items.Add(newItem2);
    ...
    protected void btn_continue_Click(object sender, EventArgs e)
    {
        ...
        if (v.Rows.Count > 0)
        {
            using (SqlCommand cmd = new SqlCommand("Check_Activation_Member_Cart", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Memberid", Request.Cookies["userid"].Value);
                ...
                {
                    Response.Redirect("Activate_Account_Checkout.aspx");
                }
