...
public partial class Register_Member_Stockist_Product_List : System.Web.UI.Page
{
                    ...
                    if (cookieExists == true)
                    {
                        if (Request.Cookies["language"].Value != null && Request.Cookies["language"].Value != "")
                        {
                            if (Request.Cookies["language"].Value == "Chinese")
                            {
                                ...
                                lbl_pacakge.InnerText = "最低购买金额 RM 150";
                                lbl_package_2.InnerText = "最低购买金额 RM 150";
                            ...
                            else
                            {
                                ...
                                lbl_pacakge.InnerText = "Minimum Purchase RM 150";
                                lbl_package_2.InnerText = "Minimum Purchase RM 150";
    ...
    protected void Load_All_Product()
    {
        ...
        cmdproduct.Parameters.AddWithValue("Member_ID", Request.QueryString["id"].ToString());
        cmdproduct.CommandType = CommandType.StoredProcedure;
        SqlDataAdapter sda = new SqlDataAdapter(cmdproduct);
        DataTable dt = new DataTable();
        sda.Fill(dt);
    ...
    {
        Response.Redirect("Register_Member_Stockist.aspx?id=" + Request.QueryString["id"].ToString());
    }
    ...
    protected void btn_continue_Click(object sender, EventArgs e)
    {
        ...
        if (v.Rows.Count > 0)
        {
            using (SqlCommand cmd = new SqlCommand("Check_Regsitration_Member_Cart", con))
            {
                ...
                {
                    Response.Redirect("Register_Member_Stockist_Checkout.aspx?id=" + Request.QueryString["id"].ToString());
                }
